...
"""
# Extract terminal quadrature masses without FGTV5.targets_to_probability's
# EPS clipping.  The clipping is useful for hurdle calculations, but can erase
# a terminal distribution when its probability is spread over many grid points.
function _raw_target_probability(f::FGTV5.FGT)
    q = Float64[]
    sizehint!(q, sum(length(b.probs) for b in f.target_bins))
    for b in f.target_bins
        for j in eachindex(b.probs)
            sc = FGTV5.scale_target_to_source(f, b.logwealth[j], b.weights[j], b.consumption)
            push!(q, sc > 0 && isfinite(sc) ? b.probs[j] / sc : 0.0)
        end
    end
    return q
end
...
function _terminal_distribution(f::FGTV5.FGT, scale::Real)
    ...
    # Small negative values can occur from Fourier truncation.  They are not
    # probability mass, so suppress only roundoff-scale negatives here.
    mass = map(qi -> qi < 0 && abs(qi) < 1e-14 ? 0.0 : qi, q)
