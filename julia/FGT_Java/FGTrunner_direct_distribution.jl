...
"""
    distribution([runner], mu, sigma, consumption, W;
                 quadrature_order=15, max_points=4000) -> wealth, mass

Return the terminal wealth distribution in ordinary wealth units.

This routine deliberately uses direct deterministic Gauss-Hermite propagation
rather than the FGT exponential-grid path.  It serves two purposes.  First, it
provides a robust terminal distribution.  Second, it is a reference calculation
against which the fast transform can be tested.

The wealth recursion is

    W[t+1] = W[t] * exp(mu[t] - sigma[t]^2/2 + sigma[t]*Z) - consumption[t+1]

with the first consumption deducted from the initial wealth before the first
return step, matching the semantics of `hurdle` and the original Java runner.

`mass` is a vector of probability atoms on an adaptively compressed wealth
support and sums to one up to floating-point error.
"""

# Standard-normal Gauss-Hermite nodes and normalized weights.  If x,w are the
# physicists' Hermite rule for exp(-x^2), then sqrt(2)x are N(0,1) nodes and the
# normalized weights sum to one.  For the Jacobi matrix below, the square of
# the first eigenvector component already gives those normalized weights.
function _normal_hermite(order::Int)
    order >= 3 || throw(ArgumentError("quadrature_order must be at least 3"))
    J = SymTridiagonal(zeros(Float64, order), sqrt.((1:order-1) ./ 2))
    E = eigen(J)
    z = sqrt(2.0) .* E.values
    w = vec(E.vectors[1, :]).^2
    w ./= sum(w)
...
# Compress a weighted discrete distribution to at most max_points atoms.  The
# bins are approximately equal-probability bins.  Each output location is the
# probability-weighted mean wealth within its bin, so total probability and
# first moment are preserved to roundoff by the compression itself.
function _compress_distribution(wealth::Vector{Float64}, mass::Vector{Float64},
                                max_points::Int)
    ...
    # Remove zero roundoff masses before sorting.
    keep = findall(>(0.0), mass)
    wealth = wealth[keep]
    mass = mass[keep]
    isempty(wealth) && return Float64[], Float64[]

    perm = sortperm(wealth)
    wealth = wealth[perm]
    mass = mass[perm]
    total = sum(mass)
    total > 0 || return Float64[], Float64[]
    mass ./= total

    length(wealth) <= max_points && return wealth, mass

    target = 1.0 / max_points
    outw = Float64[]
    outm = Float64[]
    sizehint!(outw, max_points)
    sizehint!(outm, max_points)

    accm = 0.0
    accwm = 0.0
    for i in eachindex(wealth)
        mi = mass[i]
        wi = wealth[i]
        accm += mi
        accwm += mi * wi
        if accm >= target && length(outw) < max_points - 1
            push!(outw, accwm / accm)
            push!(outm, accm)
            accm = 0.0
            accwm = 0.0
        end
...
function _series_parameter(x::Real, n::Int, name::AbstractString)
    return fill(float(x), n)
end

function _series_parameter(x::AbstractVector{<:Real}, n::Int, name::AbstractString)
    length(x) >= n || throw(DimensionMismatch("$name is shorter than consumption"))
    return Float64.(x[1:n])
end

function _direct_distribution(mu, sigma,
                              consumption::AbstractVector{<:Real}, W::Real;
                              quadrature_order::Int=15,
                              max_points::Int=4000)
    n = length(consumption)
    n == 0 && return Float64[float(W)], Float64[1.0]

    μ = _series_parameter(mu, n, "mu")
    σ = _series_parameter(sigma, n, "sigma")
    c = Float64.(consumption)
    any(<(0.0), σ) && throw(ArgumentError("sigma must be nonnegative"))

    # Match FGTrunner.java.  Consumption[1] is taken immediately.  The first
    # stochastic return then uses mu[1], sigma[1], and is followed by c[2].
    wealth = Float64[float(W) - c[1]]
    mass = Float64[1.0]
    n == 1 && return wealth, mass

    z, zw = _normal_hermite(quadrature_order)

    for t in 1:n-1
        growth = exp.(μ[t] .- 0.5 * σ[t]^2 .+ σ[t] .* z)
        m = length(wealth)
        q = length(z)
        nextw = Vector{Float64}(undef, m * q)
        nextm = Vector{Float64}(undef, m * q)

        k = 1
        @inbounds for i in eachindex(wealth)
            wi = wealth[i]
            pi = mass[i]
            for j in eachindex(z)
                nextw[k] = wi * growth[j] - c[t + 1]
                nextm[k] = pi * zw[j]
                k += 1
            end
        end

        wealth, mass = _compress_distribution(nextw, nextm, max_points)
    end
...
function distribution(::Runner, mu, sigma,
                      consumption::AbstractVector{<:Real}, W::Real;
                      quadrature_order::Int=15,
                      max_points::Int=4000)
    return _direct_distribution(mu, sigma, consumption, W;
                                quadrature_order=quadrature_order,
                                max_points=max_points)
end

distribution(mu, sigma, consumption::AbstractVector{<:Real}, W::Real; kwargs...) =
    distribution(Runner(), mu, sigma, consumption, W; kwargs...)

"""
    density([runner], mu, sigma, consumption, W; kwargs...) -> wealth, pdf

Construct a wealth-space PDF from `distribution`.  The returned values are a
piecewise-grid density estimate.  `distribution` is preferable for probability
sums and moments because it retains the actual quadrature masses.
"""
function _mass_to_density(wealth::AbstractVector{<:Real},
                          mass::AbstractVector{<:Real})
    length(wealth) == length(mass) || throw(DimensionMismatch("wealth and mass vectors differ in length"))
    length(wealth) < 2 && return Float64.(wealth), zeros(Float64, length(wealth))

    perm = sortperm(wealth)
    w = Float64.(wealth[perm])
    m = Float64.(mass[perm])
    n = length(w)

    widths = Vector{Float64}(undef, n)
    widths[1] = w[2] - w[1]
    widths[end] = w[end] - w[end-1]
...
function density(r::Runner, mu, sigma,
                 consumption::AbstractVector{<:Real}, W::Real; kwargs...)
    wealth, mass = distribution(r, mu, sigma, consumption, W; kwargs...)
    return _mass_to_density(wealth, mass)
end

density(mu, sigma, consumption::AbstractVector{<:Real}, W::Real; kwargs...) =
    density(Runner(), mu, sigma, consumption, W; kwargs...)
